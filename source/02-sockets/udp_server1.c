...
int main(int argc, char *argv[]) {
    ...
    echoserver.sin_addr.s_addr = inet_addr(argv[1]);  /* we will receive message only to that IP on server */
    //echoserver.sin_addr.s_addr = htonl(INADDR_ANY); /* We can receive requests from any IP address valid for server*/
    ...
    while (1) {
        ...
        /* we receive a message from a particular client */
